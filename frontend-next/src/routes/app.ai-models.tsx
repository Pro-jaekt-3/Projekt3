              ...
              <CardContent className="space-y-3">
                ...
                {m.defaultFor.length > 0 && (
                  <div className="text-xs text-muted-foreground">
                    Default for:{" "}
                    <span className="font-medium text-foreground">{m.defaultFor.join(", ")}</span>
                  </div>
                )}
                <div className="flex flex-wrap items-center justify-between gap-2 border-t pt-3">
                  <div className="flex items-center gap-1.5 text-xs">
                    <Activity
                      className={`h-3.5 w-3.5 ${m.lastTest.status === "ok" ? "text-emerald-600" : m.lastTest.status === "warn" ? "text-amber-600" : "text-rose-600"}`}
                    />
          ...
          <div className="overflow-x-auto">
            <Table>
              ...
              <TableBody>
                {[
                  "Question drafting",
                  "Equivalent question generation",
  ...
  return (
    <DialogContent className="max-w-lg">
      <DialogHeader>
        ...
        <DialogDescription>
          API keys are stored securely and never shown to instructors.
        </DialogDescription>
      </DialogHeader>
      <div className="space-y-3">
        <div className="grid gap-3 sm:grid-cols-2">
          <Field label="Display name">
            <Input placeholder="Local Reasoning" />
          </Field>
          ...
          <Field label="Model identifier">
            <Input placeholder="gpt-oss:120b" />
          </Field>
        ...
        <div className="space-y-2 rounded-md border bg-surface p-3">
          ...
          <label className="flex items-center justify-between text-sm">
            <span>Available to instructors</span>
            <Switch defaultChecked />
          </label>
        </div>
        <div>
        ...
        <Field label="Admin notes">
          <Textarea rows={2} placeholder="Internal note for other admins…" />
        </Field>
      </div>
      <DialogFooter>
        <Button
