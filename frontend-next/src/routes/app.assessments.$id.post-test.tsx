...
import {
  Check, ChevronLeft, ChevronRight, Sparkles, AlertTriangle, Shield, Cloud, X, Save,
} from "lucide-react";
...
function PostTestWizard() {
  ...
  const publish = () => {
    ...
    navigate({ to: "/app/assessments/$id", params: { id: "a3" }, search: { published: 1 } as never });
  };
    ...
    <>
      <PageHeader
        ...
        actions={
          <>
            ...
            <Button variant="outline" size="sm"><Save className="mr-1.5 h-4 w-4" /> Save as draft</Button>
          </>
        }
      />
      ...
      <div className="p-4 sm:p-6 lg:p-8">
        ...
        <div className="mt-6 max-w-4xl space-y-4">
          {step === 1 && (
            <Card>
              <CardHeader><CardTitle className="text-base">Choose related pre-test</CardTitle></CardHeader>
              <CardContent className="space-y-2">
                {ASSESSMENTS.filter((a) => a.type === "Pre-test").map((a) => (
                  <label key={a.id} className={cn("flex cursor-pointer items-start gap-3 rounded-md border bg-card p-4", pre.id === a.id && "border-primary bg-primary-soft/50")}>
                    <input
                      type="radio"
                      checked={pre.id === a.id}
                      onChange={() => setPre(a)}
          ...
          {step === 2 && (
            <Card>
              <CardHeader>
                ...
                <CardDescription>Same topics, objectives and difficulty as <strong>{pre.title}</strong>.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="grid gap-3 sm:grid-cols-2">
                  <BlueprintCard title="Topics" lines={["SQL Basics", "Joins", "Normalization"]} />
                  <BlueprintCard title="Difficulty" lines={["Easy 40%", "Medium 40%", "Hard 20%"]} />
                  <BlueprintCard title="Question count" lines={[`${pre.questionIds.length} questions`]} />
                ...
                <label className="flex items-start gap-2 rounded-md border bg-surface p-3 text-sm">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-muted-foreground">Slightly weight questions from SQL Joins.</div>
                  </div>
                </label>
              </CardContent>
            </Card>
          ...
          {step === 3 && (
            <Card>
              <CardHeader>
                ...
                <CardDescription>Post-tests should measure the same knowledge using equivalent but not identical questions.</CardDescription>
              </CardHeader>
                      ...
                      return (
                        <TableRow key={q.id}>
                          <TableCell className="max-w-xs">
                            ...
                            <div className="mt-1 text-xs text-muted-foreground">{q.topic} · {q.difficulty}</div>
                          </TableCell>
                          <TableCell className="max-w-xs">
                            {has ? (
                              <>
                                <span className="line-clamp-2 text-sm">
                                  {q.text.replace("Which", "Identify which").replace("SQL", "relational")}
                                </span>
                          ...
                          <TableCell className="text-right">
                            {has ? (
                              <Button size="sm" variant="outline">Use variant</Button>
                            ) : (
                              <Button size="sm" variant="ghost">Mark for AI</Button>
                            )}
                          </TableCell>
                        </TableRow>
                      );
          ...
          {step === 4 && (
            <div className="space-y-4">
              <AIPanel modelId={modelId} setModelId={setModelId} onGenerate={() => { setGenerated(true); toast("Generated 2 draft variants — review required"); }} />

              {generated ? (
                <Card>
                  <CardHeader>
                    <CardTitle className="text-base">Review AI-generated variants</CardTitle>
                    <CardDescription>All AI variants are marked <strong>Needs Review</strong>. Approve before publish.</CardDescription>
                  </CardHeader>
                        ...
                        <div className="grid gap-4 p-4 md:grid-cols-2">
                          ...
                          <div className="rounded-md border-l-2 border-primary bg-primary-soft/40 p-3">
                            <div className="flex items-center justify-between">
                              <span className="text-[10px] uppercase tracking-wide text-accent-foreground">Equivalent variant (AI draft)</span>
                              <StatusBadge status="AI generated" tone="primary" />
                            </div>
                            <div className="mt-1 text-sm font-medium">
                              {q.text.replace(/^Which/, "Identify which").replace(/SQL/g, "relational")}
                            </div>
                            <div className="mt-1 text-xs text-muted-foreground">Same topic · Same difficulty</div>
                          </div>
                        </div>
                        <div className="flex flex-wrap items-center justify-end gap-2 border-t bg-surface px-4 py-2">
                          <Button size="sm" variant="ghost" onClick={() => setReviewed({ ...reviewed, [q.id]: "rejected" })}>Reject</Button>
                          <Button size="sm" variant="outline" onClick={() => setReviewed({ ...reviewed, [q.id]: "later" })}>Mark for later</Button>
                          <Button size="sm" onClick={() => setReviewed({ ...reviewed, [q.id]: "approved" })}>
                            {reviewed[q.id] === "approved" ? <><Check className="mr-1.5 h-4 w-4" /> Approved</> : "Approve as Needs Review"}
                          </Button>
                        </div>
                      </div>
                    ))}
                ...
                <Card>
                  <CardContent className="py-10 text-center text-sm text-muted-foreground">
                    2 questions are missing equivalent variants. Click <strong>Generate draft variants with AI</strong> above.
                  </CardContent>
                </Card>
              )}
            </div>
          ...
          {step === 5 && (
            <Card>
              <CardHeader><CardTitle className="text-base">Review & publish</CardTitle></CardHeader>
              <CardContent className="space-y-4">
                <div className="grid gap-2 text-sm">
                  <Row label="Related pre-test" value={pre.title} />
                  <Row label="Blueprint" value="Reused" />
                  <Row label="Questions" value={`${pre.questionIds.length} (equivalent variants preferred)`} />
                  <Row label="Assigned to" value="Same participants as pre-test" />
                </div>
                <div className="rounded-md border bg-surface p-3 text-sm">
                  <div className="mb-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground">Comparability checklist</div>
                  <ul className="space-y-1.5">
                    <li className="flex items-center gap-2"><Check className="h-4 w-4 text-emerald-600" /> Same blueprint</li>
                    <li className="flex items-center gap-2"><Check className="h-4 w-4 text-emerald-600" /> Equivalent variants selected</li>
                    <li className="flex items-center gap-2"><AlertTriangle className="h-4 w-4 text-amber-600" /> 2 AI variants need approval</li>
                    <li className="flex items-center gap-2"><Check className="h-4 w-4 text-emerald-600" /> Participants assigned</li>
                  </ul>
                </div>
              </CardContent>
            </Card>
          ...
          <div className="flex flex-col-reverse items-stretch justify-between gap-2 sm:flex-row sm:items-center">
            ...
            {step < 5 ? (
              <Button onClick={next}>Continue <ChevronRight className="ml-1.5 h-4 w-4" /></Button>
            ) : (
      ...
      {STEPS.map((s, i) => {
        ...
        return (
          <li key={s.id} className="flex items-center gap-2 sm:flex-1">
            ...
            <span className={cn("text-xs sm:text-sm font-medium", status === "pending" && "text-muted-foreground")}>{s.title}</span>
            {i < STEPS.length - 1 && <span className="hidden flex-1 border-t border-dashed sm:block" />}
          </li>
        );
      })}
    </ol>
...
function BlueprintCard({ title, lines }: { title: string; lines: string[] }) {
  return (
    <div className="rounded-md border bg-card p-3">
      ...
      <ul className="mt-1 space-y-0.5 text-sm">{lines.map((l) => <li key={l}>{l}</li>)}</ul>
    </div>
  );
}
...
function AIPanel({ modelId, setModelId, onGenerate }: { modelId: string; setModelId: (v: string) => void; onGenerate: () => void }) {
  ...
  return (
    <Card className="border-primary/30 bg-primary-soft/30">
      ...
      <CardContent className="space-y-3">
        <div className="grid gap-3 sm:grid-cols-[260px_1fr]">
          <div>
            ...
            <Select value={modelId} onValueChange={setModelId}>
              ...
              <SelectContent>
                {enabled.map((m) => (
                  <SelectItem key={m.id} value={m.id}>{m.displayName} — {m.location}</SelectItem>
                ))}
              </SelectContent>
            </Select>
          </div>
          {model && (
            <div className="flex flex-wrap items-center gap-1.5 self-end">
              <StatusBadge status={model.location === "local" ? "Local" : "Cloud"} tone={model.location === "local" ? "success" : "warning"} icon={model.location === "local" ? <Shield className="h-3 w-3" /> : <Cloud className="h-3 w-3" />} />
              <StatusBadge status={`${model.quality}`} tone="info" />
              <StatusBadge status={`${model.speed}`} tone="muted" />
              {model.defaultFor.length > 0 && <StatusBadge status={`Recommended: ${model.defaultFor[0]}`} tone="primary" />}
            </div>
          )}
        </div>
        {model && (
        ...
        <Button onClick={onGenerate}><Sparkles className="mr-1.5 h-4 w-4" /> Generate draft variants with AI</Button>
      </CardContent>
    </Card>
  );
}
