...
const REPORT_TEMPLATES = [
  {
    ...
    desc: "1-page snapshot for leadership.",
    icon: FileText,
  },
  {
    id: "weekly",
    title: "Weekly cohort digest",
    desc: "Last 7 days of activity, scores and at-risk learners.",
    icon: Clock,
  },
  {
    id: "prepost",
    title: "Pre/Post comparison",
    desc: "Topic-level improvement after the post-test.",
    icon: TrendingUp,
  },
  {
    id: "risk",
    title: "At-risk learners",
    desc: "Participants likely to fail without intervention.",
    icon: ShieldAlert,
  },
  {
    id: "items",
    title: "Item analysis",
    desc: "Question-level quality, discrimination, and flags.",
    icon: Wand2,
  },
];
...
const AI_INSIGHTS = [
  {
    ...
    actions: [
      "Add 4 targeted practice questions",
      "Schedule a 20-min review session",
      "Re-test only weak objectives",
    ],
  },
  {
    ...
    actions: [
      "Flag 2 low-discrimination items",
      "Rewrite distractors with AI",
      "Move 1 item to medium difficulty",
    ],
  },
  {
...
const AT_RISK = [
  {
    ...
    completion: 33,
    risk: 82,
    reason: "Low completion + weak Joins score",
  },
  {
    name: "Luka Zupan",
    score: 62,
    completion: 67,
    risk: 64,
    reason: "Inconsistent attempts, struggles on hard items",
  },
  {
    name: "Jure Petrič",
    score: 70,
    completion: 67,
    risk: 41,
    reason: "Trending down vs cohort over last 2 weeks",
  },
];
...
const ITEM_FLAGS = [
  {
    id: "q-12",
    text: "Which join returns rows from both tables even when there is no match?",
  ...
  },
  {
    ...
    flag: "Low discrimination (0.11)",
    severity: "high" as const,
  },
  {
    id: "q-19",
    text: "What is the result of SELECT * FROM users LEFT JOIN orders …",
    flag: "Distractors too similar",
...
function fakeAnswer(q: string) {
  ...
  return (
    hit?.text ??
    "I scanned the cohort data but didn't find a strong signal for that. Try one of the suggested prompts, or ask about topics, at-risk learners, item quality, or trends."
  );
}
...
function AIInsights() {
  ...
  const [messages, setMessages] = useState<ChatMsg[]>([
    {
      role: "ai",
      text: "Hi 👋 I'm your analytics assistant. I've already analyzed **Introduction to Databases** — 4 assessments, 28 participants. Ask me anything, or pick a prompt below.",
    },
  ]);
  ...
  const radarData = TOPIC_PERFORMANCE.map((t) => ({
    topic: t.topic,
    current: t.score,
    target: 80,
  }));
      ...
      <div className="space-y-6 p-4 sm:p-6 lg:p-8">
        ...
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-5">
          ...
          <MetricCard
            ...
            value={`${AT_RISK.length} / ${training.participants}`}
            hint="Predicted to miss pass mark"
          />
          <MetricCard
            label="Items needing rewrite"
            value={ITEM_FLAGS.length}
          ...
          <TabsContent value="performance" className="mt-4 grid gap-4 lg:grid-cols-2">
            <Card>
              <CardHeader>
                ...
                <p className="text-xs text-muted-foreground">
                  Target = 80%. AI highlights gaps below the target ring.
                </p>
              </CardHeader>
              <CardContent>
                <div className="h-72">
            ...
            <Card>
              <CardHeader>
                ...
                <p className="text-xs text-muted-foreground">
                  AI summary: largest gain on Joins (+26 pts).
                </p>
              </CardHeader>
              <CardContent>
                <div className="h-72">
            ...
            <Card className="lg:col-span-2">
              <CardHeader>
                ...
                <p className="text-xs text-muted-foreground">
                  Red cells are AI-flagged objectives below 60%.
                </p>
              </CardHeader>
              <CardContent>
                <Heatmap />
          ...
          <TabsContent value="trends" className="mt-4 grid gap-4 lg:grid-cols-2">
            <Card>
              <CardHeader>
                ...
                <p className="text-xs text-muted-foreground">
                  AI forecast: post-test will land at 78–82%.
                </p>
              </CardHeader>
              <CardContent>
                <div className="h-72">
            ...
            <Card>
              <CardHeader>
                ...
                <p className="text-xs text-muted-foreground">
                  Practice completion correlates with +14 pts on post-test.
                </p>
              </CardHeader>
              <CardContent>
                <div className="h-72">
          ...
          <TabsContent value="distribution" className="mt-4 grid gap-4 lg:grid-cols-2">
            <Card>
              <CardHeader>
                ...
                <p className="text-xs text-muted-foreground">
                  Bimodal — 2 small clusters detected.
                </p>
              </CardHeader>
              <CardContent>
                <div className="h-72">
            ...
            <Card>
              <CardHeader>
                ...
                <p className="text-xs text-muted-foreground">
                  Hard items below the 55–65% healthy band.
                </p>
              </CardHeader>
              <CardContent>
                <div className="h-72">
                ...
                {ITEM_FLAGS.map((it) => (
                  <div key={it.id} className="rounded-md border p-3">
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <Badge
                        variant={it.severity === "high" ? "destructive" : "secondary"}
                        className="text-[10px]"
                      >
                        {it.severity}
                      </Badge>
                    ...
                    <div className="mt-2 flex gap-2">
                      <Button size="sm" variant="outline" className="h-7 text-xs">
                        <Wand2 className="mr-1 h-3 w-3" /> Rewrite with AI
                      </Button>
                      <Button size="sm" variant="ghost" className="h-7 text-xs">
                        View question <ChevronRight className="ml-1 h-3 w-3" />
                      </Button>
                    </div>
                  </div>
                ))}
        ...
        <Card>
          <CardHeader>
            ...
            <p className="text-xs text-muted-foreground">
              One-click actions, queued for instructor approval — nothing runs automatically.
            </p>
          </CardHeader>
          <CardContent className="grid gap-3 md:grid-cols-2">
            {[
              {
                t: "Rewrite 2 flagged hard items",
                d: "Improve discrimination on q-7 and q-12.",
                a: "Generate drafts",
              },
              {
                t: "Draft 4 new Joins practice items",
                d: "Targeting 'Resolve ambiguous columns'.",
                a: "Draft questions",
              },
              {
                t: "Send personalized nudges to 5 learners",
                d: "Tailored to weakest objective per learner.",
                a: "Preview messages",
              },
              {
                t: "Publish a 10-min Joins booster",
                d: "Reuse the existing practice blueprint.",
                a: "Open wizard",
              },
            ].map((s) => (
              <div
...
function Markdown({ text }: { text: string }) {
  ...
  lines.forEach((raw) => {
    ...
    if (/^#\s/.test(line)) {
      flushList();
      out.push(
        <h3 key={out.length} className="mt-2 text-base font-semibold">
      ...
      flushList();
      out.push(
        <div key={out.length} className="ml-4">
          {inline(line)}
        </div>,
      );
    } else if (/^[-*]\s/.test(line)) {
      ...
      flushList();
      out.push(<div key={out.length} className="h-1" />);
    } else {
      flushList();
      out.push(
      ...
      <table className="w-full min-w-[480px] border-separate border-spacing-1 text-xs">
        <thead>
          <tr>
            ...
            {cols.map((c) => (
              <th key={c} className="text-center font-medium text-muted-foreground">
                {c}
              </th>
            ))}
          </tr>
            ...
            <tr key={r.topic}>
              ...
              {r.cells.map((v, i) => (
                <td key={i} className="p-0">
                  <div
                    className={cn(
                      "flex h-10 items-center justify-center rounded-md font-semibold tabular-nums",
                      color(v),
                    )}
                  >
                    {v}%
